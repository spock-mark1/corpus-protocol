...
@tool("report_activity", "Report an activity to the Corpus Web dashboard")
async def report_activity(type: str, content: str, channel: str) -> dict:
    result = await _api.report_activity(
        _settings.corpus_id, type_=type, content=content, channel=channel
    )
...
async def check_approval(approval_id: str) -> dict:
    result = await _api.get_approval(_settings.corpus_id, approval_id)
    if result:

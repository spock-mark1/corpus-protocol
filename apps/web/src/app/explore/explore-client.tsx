...
export function ExploreClient({ corpuses }: { corpuses: CorpusListItem[] }) {
  ...
  const filtered = useMemo(() => {
    let result = corpuses.filter((item) => {
    ...
    return result;
  }, [corpuses, search, activeCategory, sortBy, statusFilter]);

  const onlineCount = corpuses.filter((c) => c.agentOnline).length;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="mb-10">
        ...
        <p className="text-sm text-muted mt-2">
          ...
          <span className="text-muted ml-1">/ {corpuses.length} total</span>
        </p>
      </div>
